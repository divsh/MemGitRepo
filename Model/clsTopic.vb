...
#Region "Columns for WhereClauses"
    Public Class ColumnList
        ...
        Public Shared ReadOnly DeferredTillDate As New WhereClauseBuilder.clsDateTimeColumn("DeferredTillDate", 6)
        Public Shared ReadOnly ParentTopicID As New WhereClauseBuilder.clsIntegerColumn("ParentTopicID", 7)
    End Class
#End Region

    Private Const FIELD_COUNT As Integer = 8
    ...
    Private mClonedTopic As clsTopic
...
#Region "Shared"
    ...
    Public Shared Function makeKey(ByVal appEnvironment As clsAppEnvironment, ByVal data As IObjectData) As clsKey
        ...
        If data Is Nothing Then
            Throw New ArgumentNullException("Cannot call clsTopic.makeKey when data is null.")
        End If

        If Not TypeOf (data) Is clsESPObjectData Then
            Throw New ArgumentException("clsTopic.makeKey failed as it is only possible to create a key if data is of the type clsESPObjectData.")
        End If
...
'*** End Custom Code
            ...
            Case "parenttopicid"
                mValues(ColumnList.ParentTopicID.Ordinal) = System.DBNull.Value
...
'*** End Custom Code
            ...
            Case "parenttopicid"
                IBO_isFieldNull = clsBusinessObjectUtilities.isIDFieldNull(mValues(ColumnList.ParentTopicID.Ordinal))
...
'*** End Custom Code
        ...
        Select Case clsBusinessObjectUtilities.getFieldName(clsStringUtilities.noSpaces(fldName).ToLower)
            ...
            Case "parenttopicid"
                Me.parentTopicID = CType(fldValue, Integer)
...
'*** End Custom Code
            ...
            Case "parenttopicid"
                Return Me.parentTopicID
            Case "questions"
                If arrayIdx <> -1 Then
                    Return Me.Questions.Item(arrayIdx)
                Else
                    Return Me.Questions()
                End If
            Case "parenttopic"
                Return Me.ParentTopic()
            Case "childtopics"
                If arrayIdx <> -1 Then
                    Return Me.ChildTopics.Item(arrayIdx)
                Else
                    Return Me.ChildTopics()
                End If
...
'*** End Custom Code
    ...
    'Property: ID 
    'A number that uniquely identifies this topic in the database.
...
'*** End Custom Code
    ...
    'Property: mainttime 
    'The time that this topic was last modified.
...
'*** End Custom Code
    ...
    'Property: userID 
    'The last user to modify this topic.
...
'*** End Custom Code
    ...
    'Property: parentTopicID 
    Public Overridable Property parentTopicID() As Integer
        Get
            Try
'*** Start Custom Code get_parenttopicid_start
'*** End Custom Code
                Return CType(clsBusinessObjectUtilities.getSafeNum(mValues(ColumnList.ParentTopicID.Ordinal)), Integer)
            Catch ex As Exception
'*** Start Custom Code get_parenttopicid_err
'*** End Custom Code
                Throw
            End Try
        End Get
        Set(ByVal thisParentTopicID As Integer)
            Dim isValueChanged As Boolean = False
            Dim oldDirty As Boolean = False
            Dim oldValue As Object = Nothing
            Dim oldValues As IObjectData = Nothing
'*** Start Custom Code let_parenttopicid_start
'*** End Custom Code
            isValueChanged = thisParentTopicID <> parentTopicID
            If Not (isValueChanged Or Not IBO_isStored) Then Exit Property
            Try
                mChangeStack += 1
                oldDirty = IBO_dirty
                oldValue = mValues(ColumnList.ParentTopicID.Ordinal)
                oldValues = mValues.Clone()
'*** Start Custom Code let_parenttopicid_beforechange
'*** End Custom Code
                If thisParentTopicID = 0 Then
                    mValues(ColumnList.ParentTopicID.Ordinal) = System.DBNull.Value
                Else
                    mValues(ColumnList.ParentTopicID.Ordinal) = thisParentTopicID
                End If
                IBO_dirty = True
                DirectCast(mCache, ICoherentCache).updateBusinessObjectAssociations(Me, "parentTopicID", oldValue, mValues(ColumnList.ParentTopicID.Ordinal))
'*** Start Custom Code let_parenttopicid_valuehaschanged
'*** End Custom Code
            Catch
                handleChangeException(oldValues, oldDirty)
                Throw
            ...
            End Try
            raiseChangedEvents(oldDirty)
'*** Start Custom Code let_parenttopicid_end
'*** End Custom Code
        End Set
    End Property
    ...
    Public Overridable ReadOnly Property IBO_businessClass() As clsBusinessClass Implements IBO.businessClass
        Get
            If mClass Is Nothing Then
                mClass = mMetaData.businessClass("topic")
            End If
            Return mClass
        End Get
    End Property
    ...
    '*************************************************************
    'Property: questions 
    Public Overridable ReadOnly Property questions(Optional ByVal refresh As Boolean = False) As clsBusinessObjectCollection(Of clsQuestion)
        Get
'*** Start Custom Code get_questions_start
'*** End Custom Code
                questions = mCache.GetRelatedBusinessCollection(Of clsTopic, clsQuestion)(Me, "topic", refresh)
'*** Start Custom Code get_questions_end
'*** End Custom Code
        End Get
    End Property

    'Property: parentTopic 
    Public Overridable ReadOnly Property parentTopic(Optional ByVal refresh As Boolean = False) As clsTopic
        Get
            parentTopic = Nothing
'*** Start Custom Code get_parenttopic_start
'*** End Custom Code
            parentTopic = mCache.GetRelatedBusinessObject(Of clsTopic)(Me, "parentTopic", refresh)
        End Get
    End Property

    'Property: childTopics 
    Public Overridable ReadOnly Property childTopics(Optional ByVal refresh As Boolean = False) As clsBusinessObjectCollection(Of clsTopic)
        Get
'*** Start Custom Code get_childtopics_start
'*** End Custom Code
                childTopics = mCache.GetRelatedBusinessCollection(Of clsTopic, clsTopic)(Me, "parenttopic", refresh)
'*** Start Custom Code get_childtopics_end
'*** End Custom Code
        End Get
    End Property

    'Sub: setParentTopicID 
    'sets the foreign key
    Public Overridable Sub setParentTopicID(ByVal thisID As Integer)
        parentTopicID = thisID
    End Sub

    'Function: getParentTopicID 
    'gets the foreign key
    Public Overridable Function getParentTopicID() As Integer
...
'*** End Custom Code
    ...
    Public Overridable Function getWidgetName(ByVal fieldName As String) As String
        ...
        Select Case clsStringUtilities.noSpaces(fieldName).ToLower
            ...
            Case "deferredtilldate", "3"
                getWidgetName = "kiwidget.kwcalendar" & " " & "deferredtilldate"
            Case "parenttopicid", "4"
                getWidgetName = "kiwidget.kwnumberbox" & " " & "parenttopicid"
                ' associations
            Case "questions", "5"
                getWidgetName = "kiwidget.kwcombo" & " " & "questions"
            Case "parenttopic", "6"
                getWidgetName = "kiwidget.kwcombo" & " " & "parenttopic"
            Case "childtopics", "7"
                getWidgetName = "kiwidget.kwcombo" & " " & "childtopics"
...
'*** End Custom Code
    ...
    Public Overridable Function getClonedTopic() As clsTopic
        getClonedTopic = mClonedTopic
    End Function
