...
#Region "Columns for WhereClauses"
    Public Class ColumnList
        ...
        Public Shared ReadOnly Response As New WhereClauseBuilder.clsIntegerColumn("Response", 4)
        Public Shared ReadOnly ResponseDateTime As New WhereClauseBuilder.clsDateTimeColumn("ResponseDateTime", 5)
        Public Shared ReadOnly QuestionID As New WhereClauseBuilder.clsIntegerColumn("QuestionID", 6)
    End Class
#End Region
    ...
    Private mClonedReview As clsReview
...
#Region "Shared"
    ...
    Public Shared Function makeKey(ByVal appEnvironment As clsAppEnvironment, ByVal data As IObjectData) As clsKey
        ...
        If data Is Nothing Then
            Throw New ArgumentNullException("Cannot call clsReview.makeKey when data is null.")
        End If

        If Not TypeOf (data) Is clsESPObjectData Then
            Throw New ArgumentException("clsReview.makeKey failed as it is only possible to create a key if data is of the type clsESPObjectData.")
        End If
...
'*** End Custom Code
            Case "response", "responseid"
                mValues(ColumnList.Response.Ordinal) = System.DBNull.Value
            Case "responsedatetime"
                mValues(ColumnList.ResponseDateTime.Ordinal) = System.DBNull.Value
            Case "questionid"
                mValues(ColumnList.QuestionID.Ordinal) = System.DBNull.Value
...
'*** End Custom Code
            ...
            Case "response", "responseid"
                IBO_isFieldNull = clsBusinessObjectUtilities.isFieldNull(mValues(ColumnList.Response.Ordinal))
            Case "responsedatetime"
                IBO_isFieldNull = clsBusinessObjectUtilities.isDateFieldNull(mValues(ColumnList.ResponseDateTime.Ordinal))
            Case "questionid"
                IBO_isFieldNull = clsBusinessObjectUtilities.isIDFieldNull(mValues(ColumnList.QuestionID.Ordinal))
...
'*** End Custom Code
        ...
        Select Case clsBusinessObjectUtilities.getFieldName(clsStringUtilities.noSpaces(fldName).ToLower)
            ...
            Case "userid"
                ...
                Throw New ArgumentOutOfRangeException("fldName", errMsg)
            Case "response", "responseid"
                Me.response = CType(fldValue, AltServer.clsAltEnums.response)
            Case "responsedatetime"
                Me.responseDateTime = CType(fldValue, Date)
            Case "questionid"
                Me.questionID = CType(fldValue, Integer)
...
'*** End Custom Code
    ...
    'Property: ID 
    'A number that uniquely identifies this review in the database.
...
'*** End Custom Code
    ...
    'Property: mainttime 
    'The time that this review was last modified.
...
'*** End Custom Code
    ...
    'Property: userID 
    'The last user to modify this review.
...
'*** End Custom Code
    ...
    'Property: response 
    Public Overridable Property response() As AltServer.clsAltEnums.response
        Get
            Try
'*** Start Custom Code get_response_start
'*** End Custom Code
                Return CType(clsBusinessObjectUtilities.getSafeNum(mValues(ColumnList.Response.Ordinal)), AltServer.clsAltEnums.response)
            Catch ex As Exception
'*** Start Custom Code get_response_err
'*** End Custom Code
                Throw
            End Try
        End Get
        Set(ByVal thisResponse As AltServer.clsAltEnums.response)
            Dim isValueChanged As Boolean = False
            Dim oldDirty As Boolean = False
            Dim oldValues As IObjectData = Nothing
'*** Start Custom Code let_response_start
'*** End Custom Code
            isValueChanged = clsBusinessObjectUtilities.hasValueChanged(Me, "response", mValues(ColumnList.Response.Ordinal), thisResponse)
            If Not (isValueChanged Or Not IBO_isStored) Then Exit Property
            Try
                mChangeStack += 1
                oldDirty = IBO_dirty
                oldValues = mValues.Clone()
                mValues(ColumnList.Response.Ordinal) = thisResponse
                mMetaData.validateAttribute(Me, mMetaData.businessAttribute(IBO_businessClass, "response"))
                IBO_dirty = True
'*** Start Custom Code let_response_valuehaschanged
'*** End Custom Code
            Catch
    ...
    ''' <summary>
    ''' This helps with enumeration text
    ''' </summary>
    Public Overridable ReadOnly Property ResponseText(Optional ByVal translate As Boolean = True) As String
        Get
            ResponseText = ""
            Try
                Select Case Me.response
                    Case AltServer.clsAltEnums.response.rvBright
                        ResponseText = "bright"
                    Case AltServer.clsAltEnums.response.rvGood
                        ResponseText = "good"
                    Case AltServer.clsAltEnums.response.rvPass
                        ResponseText = "pass"
                    Case AltServer.clsAltEnums.response.rvBad
                        ResponseText = "bad"
                    Case AltServer.clsAltEnums.response.rvNull
                        ResponseText = "null"
                    Case AltServer.clsAltEnums.response.rvCancel
                        ResponseText = "cancel"
                End Select

                If translate Then
                    ResponseText = mAppEnvironment.InfController.langController("alt").translate(ResponseText, gInfTranslationIdBuilder.getIdForEnumItemName("response", ResponseText))
                End If
            Catch ex As Exception
'*** Start Custom Code get_responsetext_err
'*** End Custom Code
                Throw
            End Try
        End Get
    ...
    'Property: responseDateTime 
    Public Overridable Property responseDateTime() As Date
        Get
            Try
'*** Start Custom Code get_responsedatetime_start
'*** End Custom Code
                Return clsBusinessObjectUtilities.getSafeDate(mValues(ColumnList.ResponseDateTime.Ordinal))
            Catch ex As Exception
'*** Start Custom Code get_responsedatetime_err
'*** End Custom Code
                Throw
            End Try
        End Get
        Set(ByVal thisResponseDateTime As Date)
            Dim isValueChanged As Boolean = False
            Dim oldDirty As Boolean = False
            Dim oldValues As IObjectData = Nothing
'*** Start Custom Code let_responsedatetime_start
'*** End Custom Code
            isValueChanged = clsBusinessObjectUtilities.hasValueChanged(Me, "responseDateTime", mValues(ColumnList.ResponseDateTime.Ordinal), thisResponseDateTime)
            If Not (isValueChanged Or Not IBO_isStored) Then Exit Property
            Try
                mChangeStack += 1
                oldDirty = IBO_dirty
                oldValues = mValues.Clone()
                mValues(ColumnList.ResponseDateTime.Ordinal) = thisResponseDateTime
                mMetaData.validateAttribute(Me, mMetaData.businessAttribute(IBO_businessClass, "responseDateTime"))
                IBO_dirty = True
'*** Start Custom Code let_responsedatetime_valuehaschanged
'*** End Custom Code
            Catch
                handleChangeException(oldValues, oldDirty)
                Throw
    ...
    Public Overridable Property questionID() As Integer
        Get
            Try
'*** Start Custom Code get_questionid_start
'*** End Custom Code
                Return CType(clsBusinessObjectUtilities.getSafeNum(mValues(ColumnList.QuestionID.Ordinal)), Integer)
            Catch ex As Exception
'*** Start Custom Code get_questionid_err
'*** End Custom Code
                Throw
            End Try
        End Get
        Set(ByVal thisQuestionID As Integer)
            Dim isValueChanged As Boolean = False
            Dim oldDirty As Boolean = False
            Dim oldValue As Object = Nothing
            Dim oldValues As IObjectData = Nothing
'*** Start Custom Code let_questionid_start
'*** End Custom Code
            isValueChanged = thisQuestionID <> questionID
            If Not (isValueChanged Or Not IBO_isStored) Then Exit Property
            Try
                mChangeStack += 1
                oldDirty = IBO_dirty
                oldValue = mValues(ColumnList.QuestionID.Ordinal)
                oldValues = mValues.Clone()
'*** Start Custom Code let_questionid_beforechange
'*** End Custom Code
                If thisQuestionID = 0 Then
                    mValues(ColumnList.QuestionID.Ordinal) = System.DBNull.Value
                Else
                    mValues(ColumnList.QuestionID.Ordinal) = thisQuestionID
                End If
                IBO_dirty = True
                DirectCast(mCache, ICoherentCache).updateBusinessObjectAssociations(Me, "questionID", oldValue, mValues(ColumnList.QuestionID.Ordinal))
'*** Start Custom Code let_questionid_valuehaschanged
'*** End Custom Code
            Catch
                handleChangeException(oldValues, oldDirty)
                Throw
            ...
            End Try
            raiseChangedEvents(oldDirty)
'*** Start Custom Code let_questionid_end
'*** End Custom Code
        End Set
    End Property
    ...
    Public Overridable ReadOnly Property IBO_businessClass() As clsBusinessClass Implements IBO.businessClass
        Get
            If mClass Is Nothing Then
                mClass = mMetaData.businessClass("review")
            End If
            Return mClass
        End Get
    End Property
    ...
    '*************************************************************
    'Property: question 
    Public Overridable ReadOnly Property question(Optional ByVal refresh As Boolean = False) As clsQuestion
        Get
            question = Nothing
'*** Start Custom Code get_question_start
'*** End Custom Code
            question = mCache.GetRelatedBusinessObject(Of clsQuestion)(Me, "question", refresh)
        End Get
    End Property

    'Sub: setQuestionID 
    'sets the foreign key
    Public Overridable Sub setQuestionID(ByVal thisID As Integer)
        questionID = thisID
    End Sub

    'Function: getQuestionID 
    'gets the foreign key
    Public Overridable Function getQuestionID() As Integer
        Return questionID
    End Function
...
'*** End Custom Code
    ...
    Public Overridable Function getWidgetName(ByVal fieldName As String) As String
        ...
        Select Case clsStringUtilities.noSpaces(fieldName).ToLower
            Case "response", "1"
                getWidgetName = "kiwidget.kwcombo" & " " & "response"
            Case "responsedatetime", "2"
                getWidgetName = "kiwidget.kwcalendar" & " " & "responsedatetime"
            Case "questionid", "3"
                getWidgetName = "kiwidget.kwnumberbox" & " " & "questionid"
                ' associations
            Case "question", "4"
                getWidgetName = "kiwidget.kwcombo" & " " & "question"
...
'*** End Custom Code
    ...
    Public Overridable Function getClonedReview() As clsReview
        getClonedReview = mClonedReview
    End Function
    ...
    Private Sub Setup(ByVal aCache As ICache)
        ...
        mValues(ColumnList.Response.Ordinal) = 0
